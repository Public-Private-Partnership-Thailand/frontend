...
function projectToFormValues(project: ProjectData): Partial<ProjectFormData> {
  return {
    title: project.title ?? '',
    ...
    sector: Array.isArray(project.sector)
      ? project.sector.map((c) => (typeof c === 'object' && c !== null ? c.id : String(c))).filter(Boolean)
      : [],
  ...
  const onSubmit: SubmitHandler<ProjectFormData> = async (data) => {
    ...
    try {
      ...
      const sector = Array.isArray(data.sector)
        ? data.sector
            .filter((s) => s)
            .map((s) => (typeof s === 'string' ? s : (s && typeof s === 'object' ? (s as { id?: string }).id : '')))
            .filter((s): s is string => Boolean(s && typeof s === 'string' && isValidBusinessGroupCode(s)))
        : []
      ...
      <div className={`mx-auto ${currentStep === 4 || currentStep === 5 ? 'max-w-[700px] md:w-[700px]' : 'max-w-[700px]'}`}>
              ...
              {[1, 2, 3, 4, 5].map((step, index) => (
                <div key={step} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${currentStep >= step ? 'bg-theme-primary text-white' : 'bg-gray-200 text-gray-600'}`}
                    >
                      {step}
                    </div>
                  ...
                  {index < 4 && (
                    <div className={`flex-1 h-0.5 mx-1 ${currentStep > step ? 'bg-theme-primary' : 'bg-gray-300'}`} />
                  )}
                </div>
              ))}
            </div>
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault()
            e.stopPropagation()
            if (currentStep === 5 && !isSubmitting) handleSubmit(onSubmit)(e)
          }}
          className="space-y-8"
          noValidate
        >
              ...
              {currentStep < 5 ? (
                ...
                <button
                  ...
                  onClick={(e) => {
                    ...
                    e.preventDefault()
                    e.stopPropagation()
                    if (currentStep === 5 && !isSubmitting && submitStatus !== 'success') handleSubmit(onSubmit)(e)
